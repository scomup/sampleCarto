...
#ifndef SAMPLE_CARTO_CORE_SCAN_MATCHING_OCCUPIED_SPACE_COST_FUNCTOR_H_
#define SAMPLE_CARTO_CORE_SCAN_MATCHING_OCCUPIED_SPACE_COST_FUNCTOR_H_
...
class OccupiedSpaceCostFunctor {
 public:
  ...
  bool operator()(const T* const pose, T* residual) const {
    ...
    const map::MapLimits& limits = probability_grid_.limits();
 ...
 private:
  ...
  class GridArrayAdapter {
   public:
    ...
    explicit GridArrayAdapter(const map::ProbabilityGrid& probability_grid)
        : probability_grid_(probability_grid) {}

    void GetValue(const int row, const int column, double* const value) const {
      if (row < kPadding || column < kPadding || row >= NumRows() - kPadding ||
          column >= NumCols() - kPadding) {
        *value = map::kMinProbability;
      } else {
   ...
   private:
    const map::ProbabilityGrid& probability_grid_;
  };
...
#endif  // SAMPLE_CARTO_CORE_SCAN_MATCHING_OCCUPIED_SPACE_COST_FUNCTOR_H_
